...
Giao diện **song ngữ Anh/Việt** (tự nhận theo ngôn ngữ tài khoản, có nút chuyển trong sidebar).
...
- Header ghi theo ngôn ngữ đang chọn trong sidebar.
...
4. Bấm **Save** (💾).
5. Quay lại tab Google Sheet, **tải lại trang (F5)**. Sau vài giây sẽ thấy menu mới **📤 Upload ảnh**.
...
3. Bấm **Chọn thư mục ảnh** → chọn folder trên máy. Ô "Tên folder" tự điền theo tên folder đó (sửa được).
...
| File | Vai trò |
|------|---------|
| `Code.gs` | Logic server: tạo folder, upload, set public, ghi Sheet |
| `Sidebar.html` | Giao diện chọn folder + tiến trình (chạy trong trình duyệt) |
| `appsscript.json` | Khai báo quyền (OAuth scopes) & múi giờ |
